...
class BbsiCloseAccountController @Inject()(
  ...
  def futureDateValidation: (LocalDate => Boolean, String) =
    ((date: LocalDate) => !date.isAfter(LocalDate.now()), Messages("tai.date.error.future"))

  def captureCloseDate(id: Int): Action[AnyContent] = (authenticate andThen validatePerson).async { implicit request =>
      ...
      bankAccount match {
        ...
        case Some(_) =>
          throw new RuntimeException(
            s"Bank account does not contain name, number or sortcode for nino: [${user.getNino}] and id: [$id]")
  ...
  def submitCloseDate(id: Int): Action[AnyContent] = (authenticate andThen validatePerson).async { implicit request =>
    implicit val user = request.taiUser

    bbsiService.bankAccount(Nino(user.getNino), id) flatMap {
      case Some(BankAccount(_, Some(_), Some(_), Some(bankName), _, _)) =>
        DateForm(Seq(futureDateValidation), Messages("tai.closeBankAccount.closeDateForm.blankDate", bankName)).form
          .bindFromRequest()
          .fold(
            formWithErrors => {
              Future.successful(BadRequest(
                views.html.incomes.bbsi.close.bank_building_society_close_date(formWithErrors, bankName, id)))
            },
            date => {
              journeyCacheService
                .cache(Map(CloseBankAccountDateKey -> date.toString, CloseBankAccountNameKey -> bankName))
                .map(_ =>
      ...
      case Some(_) =>
        Future.successful(
          internalServerError(
            s"Bank account does not contain name, number or sortcode for nino: [${user.getNino}] and id: [$id]"))
      case None =>
        Future.successful(internalServerError(s"Bank account not found for nino: [${user.getNino}] and id: [$id]"))
    }
  }
    ...
    implicit request =>
      ...
      (for {
        ...
        bankAccount match {
          case Some(BankAccount(_, Some(_), Some(_), Some(bankName), _, _)) =>
            Ok(
              views.html.incomes.bbsi.close.bank_building_society_closing_interest(
                id,
                BankAccountClosingInterestForm.form.fill(
  ...
  def submitClosingInterest(id: Int): Action[AnyContent] = (authenticate andThen validatePerson).async {
    implicit request =>
      ...
      BankAccountClosingInterestForm.form
        .bindFromRequest()
        .fold(
          formWithErrors => {
            Future.successful(
              BadRequest(views.html.incomes.bbsi.close.bank_building_society_closing_interest(id, formWithErrors)))
          },
          form => {
            journeyCacheService.cache(Map(
              CloseBankAccountInterestKey    -> FormHelper.stripNumber(form.closingInterestEntry.getOrElse("")),
              CloseBankAccountInterestChoice -> form.closingBankAccountInterestChoice.getOrElse("")
            )) map { _ =>
              Redirect(controllers.income.bbsi.routes.BbsiCloseAccountController.checkYourAnswers(id))
            }
          }
  ...
  def checkYourAnswers(id: Int): Action[AnyContent] = (authenticate andThen validatePerson).async { implicit request =>
    ...
    journeyCacheService
      .collectedValues(Seq(CloseBankAccountDateKey), Seq(CloseBankAccountNameKey, CloseBankAccountInterestKey)) map {
      seq =>
