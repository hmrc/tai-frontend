...
case class PensionDuplicateSubmissionWarningViewModel(incomeSource: String) extends DuplicateSubmissionWarning(incomeSource) {

  override def warningHeading(implicit messages: Messages): String = {
    messages("tai.pension.warning.heading", incomeSource)
  }

  override def yesChoiceText(implicit messages: Messages): String = {
    messages("tai.pension.warning.radio1", incomeSource)
  }
  ...
  override def actionRoute = {controllers.pensions.routes.UpdatePensionProviderController.submitDuplicateSubmissionWarning}
}
