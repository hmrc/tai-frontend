...
object YourIncomeCalculationHelper {
  ...
  def displayPayrollNumber(hasDuplicateEmploymentNames: Boolean, worksNumber: Option[String], isPension: Boolean): Option[String] = {
  ...
  def getCurrentYearPayments(taxSummaryDetails: TaxSummaryDetails, empId: Int): (List[RtiPayment], Boolean, Option[String]) = {
  ...
  def getIncomeExplanationMessage(incomeExplanation: IncomeExplanation): (Option[String], Option[String]) = {
  ...
  def getSameMsg(incomeExplanation: IncomeExplanation): (Option[String], Option[String]) = {
    ...
    if (incomeExplanation.calcAmount.isDefined && (incomeExplanation.calcAmount.getOrElse(BigDecimal(0)) == incomeExplanation.payToDate)) {
      ...
      (Some(Messages("tai.income.calculation.rti." + (if (incomeExplanation.isPension) "pension" else "emp") +
        ".same", startDate.map(_.toString("d MMMM yyyy")).getOrElse(""),
        incomeExplanation.paymentDate.map(_.toString("d MMMM yyyy")).getOrElse(""), MoneyPounds(incomeExplanation.payToDate, 0).quantity)), None)
    } else { (None, None) }
  }

  def getCeasedMsg(incomeExplanation: IncomeExplanation): (Option[String], Option[String]) = {
    ...
    (incomeExplanation.employmentStatus.getOrElse(BigDecimal(1)), incomeExplanation.endDate.isDefined,
      ...
      case (EmploymentStatus.Ceased.code, true, true) =>
        (Some(Messages("tai.income.calculation.rti.ceased." + (if (incomeExplanation.isPension) "pension" else "emp"),
          incomeExplanation.endDate.map(_.toString("d MMMM yyyy")).getOrElse(""))), None)
  ...
  def getPayFreqMsg(incomeExplanation: IncomeExplanation): (Option[String], Option[String]) = {
    ...
    (incomeExplanation.payFrequency, isMidYear, incomeExplanation.calcAmount.isDefined) match {
      case ((Some(Weekly) | Some(Fortnightly) | Some(FourWeekly) | Some(Monthly) | Some(Quarterly) | Some(BiAnnually)), false, true) =>
        (Some(Messages("tai.income.calculation.rti.continuous.weekly." + pensionOrEmp,
          MoneyPounds(incomeExplanation.payToDate, 2).quantity, incomeExplanation.paymentDate.map(_.toString("d MMMM yyyy")).getOrElse(""))),
      ...
      case ((Some(Weekly) | Some(Fortnightly) | Some(FourWeekly) | Some(Monthly) | Some(Quarterly) | Some(BiAnnually) | Some(Annually)), true, true) =>
        (Some(Messages("tai.income.calculation.rti.midYear.weekly", incomeExplanation.startDate.map(_.toString("d MMMM yyyy")).getOrElse(""),
          incomeExplanation.paymentDate.map(_.toString("d MMMM yyyy")).getOrElse(""), MoneyPounds(incomeExplanation.payToDate, 2).quantity)),
          Some(Messages("tai.income.calculation.rti." + pensionOrEmp + ".estimate",
            MoneyPounds(incomeExplanation.calcAmount.getOrElse(BigDecimal(0)), 0).quantity)))
  ...
  def getManualUpdateMsg(incomeExpl: IncomeExplanation): (Option[String], Option[String]) = {
    ...
    (incomeExpl.iabdSource.getOrElse(0), incomeExpl.notificationDate.isDefined, incomeExpl.updateActionDate.isDefined) match {
      case (IabdUpdateSource.ManualTelephone.code, true, true) => (Some(Messages("tai.income.calculation.manual.update.phone",
        incomeExpl.updateActionDate.get.toString("d MMMM yyyy"), incomeExpl.notificationDate.get.toString("d MMMM yyyy"))),
      ...
      case (IabdUpdateSource.OtherForm.code | IabdUpdateSource.InformationLetter.code, true, true) =>
        (Some(Messages("tai.income.calculation.manual.update.informationLetter", incomeExpl.updateActionDate.get.toString("d MMMM yyyy"),
          incomeExpl.notificationDate.get.toString("d MMMM yyyy"))),
          Some(Messages("tai.income.calculation.rti.manual.update.estimate", incomeExpl.grossAmount.getOrElse(BigDecimal(0)))))
  ...
  def getPreviousYearPayments(sessionDetails: SessionData, empId: Int):
  ...
  private def formHtml(isPension: Option[Boolean]): Html = {
  ...
  def getNotMatchingTotalMessage(employmentPayments: List[RtiPayment], isPension: Option[Boolean]): Option[String] = {
  ...
  def getEyuMessage(rtiEyu: List[RtiEyu], employerName: String) = {
      ...
      eyuObject =>
        ...
        (eyuProperty, eyuAmount, date, eyuObjectList.size) match {
          case ("TaxablePayDelta", amount, date, 1) =>
            messages += Messages("tai.income.calculation.eyu.single.taxableincome", date.toString(EYU_DATE_FORMAT), lessOrMore(amount))
          case ("TotalTaxDelta", amount, date, 1) =>
            messages += Messages("tai.income.calculation.eyu.single.taxPaid", date.toString(EYU_DATE_FORMAT), lessOrMore(amount))
          case ("EmpeeContribnsDelta", amount, date, 1) =>
            messages += Messages("tai.income.calculation.eyu.single.nationalInsurance", date.toString(EYU_DATE_FORMAT), lessOrMore(amount))
          case ("TaxablePayDelta", amount, date, size) if size > 1 =>
            messages += Messages("tai.income.calculation.eyu.multi.taxableincome", date.toString(EYU_DATE_FORMAT), lessOrMore(amount))
          case ("TotalTaxDelta", amount, date, size) if size > 1 =>
            messages += Messages("tai.income.calculation.eyu.multi.taxPaid", date.toString(EYU_DATE_FORMAT), lessOrMore(amount))
          case ("EmpeeContribnsDelta", amount, date, size) if size > 1 =>
            messages += Messages("tai.income.calculation.eyu.multi.nationalInsurance", date.toString(EYU_DATE_FORMAT), lessOrMore(amount))
          case _ => messages
        }
    }
