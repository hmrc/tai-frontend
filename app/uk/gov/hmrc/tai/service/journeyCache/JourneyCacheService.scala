...
class JourneyCacheService @Inject()(val journeyName: String, journeyCacheConnector: JourneyCacheConnector)
  ...
  def mandatoryJourneyValues(keys: String*)(
    implicit hc: HeaderCarrier,
    executionContext: ExecutionContext): Future[Either[String, Seq[String]]] =
    for {
      cache <- currentCache
    } yield {
  ...
  def optionalValues(
    keys: String*)(implicit hc: HeaderCarrier, executionContext: ExecutionContext): Future[Seq[Option[String]]] =
  ...
  def collectedJourneyValues(mandatoryJourneyValues: Seq[String], optionalValues: Seq[String])(
    implicit hc: HeaderCarrier,
    executionContext: ExecutionContext): Future[Either[String, (Seq[String], Seq[Option[String]])]] =
    for {
      cache <- currentCache
    } yield {
