...
class CompanyCarService @Inject()(
  carConnector: CompanyCarConnector,
  ...
  private def cacheFromCompanyCar(
    employmentOpt: Option[Employment],
    ccBen: CompanyCarBenefit,
    empId: Int): Option[Map[String, String]] =
    ccBen.companyCars.headOption flatMap { car =>
      ...
      val optionalStart = car.dateMadeAvailable map { dt =>
        CompanyCar_DateStartedKey -> dt.toString
      }
      val optionalFuelStart = car.dateActiveFuelBenefitMadeAvailable map { dt =>
        CompanyCar_DateFuelBenefitStartedKey -> dt.toString
      }
      Some(
        (Seq(
  ...
  def withdrawCompanyCarAndFuel(nino: Nino, referer: String)(implicit hc: HeaderCarrier): Future[TaiResponse] =
    journeyCacheService.currentCache.flatMap { cache =>
      val taiResponse: Option[Future[TaiResponse]] = for {
        ...
        carConnector
          .withdrawCompanyCarAndFuel(
            nino,
            employmentSeqNum.toInt,
            carSeqNum.toInt,
            WithdrawCarAndFuel(version.toInt, new LocalDate(carEndDate), fuelDate.map(x => new LocalDate(x))))
          .map { r =>
            val isSuccess = r match {
              case TaiSuccessResponse => true
              case _                  => false
            }
            auditService.sendEndCompanyCarAuditEvent(
              nino.toString(),
              employmentSeqNum,
              carSeqNum,
              carEndDate,
