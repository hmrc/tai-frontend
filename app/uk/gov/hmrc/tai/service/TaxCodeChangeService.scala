...
class TaxCodeChangeService @Inject() (taxCodeChangeConnector: TaxCodeChangeConnector) extends Logging {
  ...
  def hasTaxCodeChanged(nino: Nino)(
    ...
    executionContext: ExecutionContext): Future[Either[TaxCodeError, HasTaxCodeChanged]] = {
  ...
  def hasTaxCodeRecordsInYearPerEmployment(nino: Nino, year: TaxYear)(
    implicit hc: HeaderCarrier,
  ...
  def latestTaxCodeChangeDate(
    nino: Nino)(implicit hc: HeaderCarrier, executionContext: ExecutionContext): Future[LocalDate] =
  ...
  private def taxCodeChanged(
    nino: Nino)(implicit hc: HeaderCarrier, executionContext: ExecutionContext): Future[Either[TaxCodeError, Boolean]] =
