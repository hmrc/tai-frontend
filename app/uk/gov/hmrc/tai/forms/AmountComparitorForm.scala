...
case class AmountComparitorForm(income: Option[String])

object AmountComparitorForm {
  implicit val formats = Json.format[AmountComparitorForm]

  def createForm(latestPayDate: Option[String] = None,
                 taxablePayYTD: Option[BigDecimal] = None)
                (implicit messages: Messages): Form[AmountComparitorForm] = {

    val fallbackDate = LocalDate.now().toString(MONTH_AND_YEAR)

    val theDate = latestPayDate.fold(fallbackDate)(identity)

    Form[AmountComparitorForm](
      ...
      ))(AmountComparitorForm.apply)(AmountComparitorForm.unapply)
    )
  }
}
