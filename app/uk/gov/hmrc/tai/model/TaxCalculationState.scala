...
case class TaxCalculationPaymentPaidState(amount: BigDecimal) extends TaxCalculationState
case class TaxCalculationPaymentChequeSentState(amount: BigDecimal) extends TaxCalculationState
...
object TaxCalculationState {
  ...
  def buildFromTaxCalculation(taxCalculation: Option[TaxCalculation]): TaxCalculationState = {
    ...
    taxCalculation match {
      case Some(TaxCalculation("Overpaid", amount, taxYear, Some("REFUND"))) =>
        TaxCalculationRefundState(amount,  taxYear, taxYear + 1)
      case Some(TaxCalculation("Overpaid", amount, _, Some("PAYMENT_PROCESSING"))) =>
        TaxCalculationPaymentProcessingState(amount)
      case Some(TaxCalculation("Overpaid", amount, _, Some("PAYMENT_PAID"))) =>
        TaxCalculationPaymentPaidState(amount)
      case Some(TaxCalculation("Overpaid", amount, _, Some("CHEQUE_SENT"))) =>
        TaxCalculationPaymentChequeSentState(amount)
      case Some(TaxCalculation("Underpaid", amount, taxYear, None)) =>
        TaxCalculationUnderpaymentState(amount, taxYear, taxYear + 1)
