  ...
  "PayPeriodForm" must {
    ...
    "return a mandatory field error" when {
      "no payPeriod is sent" in {
        ...
        payPeriodForm.errors must contain(FormError("payPeriod",
          List(Messages("tai.payPeriod.error.form.incomes.radioButton.mandatory")))
        )
      }
      ...
      "provided with a 'other' payPeriod with no number of days passed" in {
        ...
        payPeriodForm.errors must contain(FormError("otherInDays",
          List(Messages("tai.payPeriod.error.form.incomes.other.mandatory")))
        )
      }
