...
class EmploymentAddDateFormSpec extends PlaySpec
  with OneAppPerSuite
  with I18nSupport {
  ...
  "EmploymentAddDateForm" must {
    ...
    "return an error" when {
      "day is blank" in {
        ...
        validatedFormNoDayError.errors must contain(FormError(DayTag, List(Messages("tai.add.date.error.blank", "employer"))))
      }

      "month is blank" in {
        val validatedFormNoMonthError = form.bind(invalidNoMonthValue)

        validatedFormNoMonthError.errors must contain(FormError(DayTag, List(Messages("tai.add.date.error.blank", "employer"))))
      }

      "year is blank" in {
        val validatedFormNoYearError = form.bind(invalidNoYearValue)

        validatedFormNoYearError.errors must contain(FormError(DayTag, List(Messages("tai.add.date.error.blank", "employer"))))
      }

      "multiple fields are blank" in {
        val validatedFormNoDayNoYearError = form.bind(invalidNoDayNoYearValue)

        validatedFormNoDayNoYearError.errors must be(List(FormError(DayTag, List(Messages("tai.add.date.error.blank", "employer")))))
      }
