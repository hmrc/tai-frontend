  ...
  "BenefitsService" when {
    "benefits" must {
      ...
      "return benefits if valid data is retrieved from the connector" in {
        when(mockBenefitsConnector.benefits(any(), any())(any(), any()))
          .thenReturn(
    ...
    "endedCompanyBenefit" must {
      "return an envelope id" in {
        ...
        when(mockBenefitsConnector.endedCompanyBenefit(meq(nino), meq(1), meq(endedCompanyBenefit))(any(), any()))
          .thenReturn(
            EitherT[Future, UpstreamErrorResponse, HttpResponse](
      ...
      "return a Left if the response contains no data to generate an envelope id" in {
        ...
        when(mockBenefitsConnector.endedCompanyBenefit(meq(nino), meq(1), meq(endedCompanyBenefit))(any(), any()))
          .thenReturn(
            EitherT[Future, UpstreamErrorResponse, HttpResponse](
              Future.successful(Right(HttpResponse(OK, Json.obj("test" -> "").toString)))
            )
          )
