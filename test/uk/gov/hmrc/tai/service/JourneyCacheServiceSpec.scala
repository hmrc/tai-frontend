...
class JourneyCacheServiceSpec extends PlaySpec
  ...
  "current value methods" must {
    ...
    "return the cached value as an instance of the relevant type" in {
      val sut = createSut
      when(sut.journeyCacheConnector.currentValueAs[String](Matchers.eq(sut.journeyName), Matchers.eq("stringKey"), any())(any())).thenReturn(Future.successful(Some("found")))
      when(sut.journeyCacheConnector.currentValueAs[Int](Matchers.eq(sut.journeyName), Matchers.eq("intKey"), any())(any())).thenReturn(Future.successful(Some(1)))
      when(sut.journeyCacheConnector.currentValueAs[Boolean](Matchers.eq(sut.journeyName), Matchers.eq("boolKey"), any())(any())).thenReturn(Future.successful(Some(true)))
      when(sut.journeyCacheConnector.currentValueAs[LocalDate](Matchers.eq(sut.journeyName), Matchers.eq("dateKey"), any())(any())).thenReturn(Future.successful(Some(LocalDate.parse("2017-10-10"))))
  ...
  "mandatory value methods" must {
    ...
    "return the value where found" in {
      val sut = createSut
      when(sut.journeyCacheConnector.mandatoryValueAs[String](Matchers.eq(sut.journeyName), Matchers.eq("stringKey"), any())(any())).thenReturn(Future.successful("found"))
      when(sut.journeyCacheConnector.mandatoryValueAs[Int](Matchers.eq(sut.journeyName), Matchers.eq("intKey"), any())(any())).thenReturn(Future.successful(1))
      when(sut.journeyCacheConnector.mandatoryValueAs[Boolean](Matchers.eq(sut.journeyName), Matchers.eq("boolKey"), any())(any())).thenReturn(Future.successful(true))
      when(sut.journeyCacheConnector.mandatoryValueAs[LocalDate](Matchers.eq(sut.journeyName), Matchers.eq("dateKey"), any())(any())).thenReturn(Future.successful(LocalDate.parse("2017-10-10")))
    ...
    "throw a runtime where not found" in {
      ...
      when(sut.journeyCacheConnector.mandatoryValueAs[String](Matchers.eq(sut.journeyName), Matchers.eq("stringKey"), any())(any())).thenReturn(failed)
      when(sut.journeyCacheConnector.mandatoryValueAs[Int](Matchers.eq(sut.journeyName), Matchers.eq("intKey"), any())(any())).thenReturn(failed)
      when(sut.journeyCacheConnector.mandatoryValueAs[Boolean](Matchers.eq(sut.journeyName), Matchers.eq("boolKey"), any())(any())).thenReturn(failed)
      when(sut.journeyCacheConnector.mandatoryValueAs[LocalDate](Matchers.eq(sut.journeyName), Matchers.eq("dateKey"), any())(any())).thenReturn(failed)
  ...
  "collectedOptionalValues" must{
    "return sequence of strings when we have values in cache" in {
      ...
      Await.result(sut.collectedOptionalValues(Seq("key1", "key2")), 5 seconds) mustBe Seq(Some("val1"), Some("val2"))
    }
    "return sequence of strings and a None when we have one value as string and a none" in {
      val sut = createSut
      when(sut.journeyCacheConnector.currentCache(Matchers.eq(sut.journeyName))(any())).thenReturn(Future.successful(testCache))

      Await.result(sut.collectedOptionalValues(Seq("key4", "key3")), 5 seconds) mustBe Seq(Some("val3"), None)
    }
  }
  ...
  "flush the cache" must {
    "remove the cache" in {
      val sut = createSut
      when(sut.journeyCacheConnector.flush(Matchers.eq(sut.journeyName))(any())).thenReturn(Future.successful(TaiSuccessResponse))
  ...
  private class SUT extends JourneyCacheService {
    override val journeyName: String = "fakeJourneyName"
    override val journeyCacheConnector: JourneyCacheConnector = mock[JourneyCacheConnector]
  }
