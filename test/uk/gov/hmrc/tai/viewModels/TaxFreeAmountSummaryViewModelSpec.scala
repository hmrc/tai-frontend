...
class TaxFreeAmountSummaryViewModelSpec extends PlaySpec with FakeTaiPlayApplication with I18nSupport {
  ...
  "TaxFreeAmountSummaryRowViewModel apply method" must {
    "build a label generated specifically to the coding component" in {
      val personalAllowance = TaxFreeAmountSummaryRowViewModel(CodingComponent(PersonalAllowancePA, Some(123), 111, "PersonalAllowancePA"), employmentNames, companyCarBenefits, totalTax)
      val foreignDividendIncome = TaxFreeAmountSummaryRowViewModel(CodingComponent(ForeignDividendIncome, Some(123), 111, "ForeignDividendIncome"), employmentNames, companyCarBenefits, totalTax)
    ...
    "build a value formatted with pound prefix" when {
      "the value in negative" in {
        val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(ForeignDividendIncome, Some(123), -11500, "ForeignDividendIncome"),
          employmentNames, companyCarBenefits, totalTax)
    ...
    "build a NOT displayable link" when {
      "coding component is another type" in {
        val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(DividendTax, Some(1), 1, "HotelAndMealExpenses"), employmentNames, companyCarBenefits, totalTax)
    ...
    "build a displayable Medical Insurance LinkViewModel" when {
      "coding component is Medical Insurance" in {
        val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(MedicalInsurance, Some(234), 11500, "MedicalInsurance"), employmentNames, companyCarBenefits, totalTax)
    ...
    "build a displayable Marriage Allowance LinkViewModel" when {
      "coding component is Marriage Allowance Received" in {
        val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(MarriageAllowanceReceived, Some(234), 11500, "MarriageAllowanceReceived"), employmentNames, companyCarBenefits, totalTax)
      ...
      "coding component is Marriage Allowance Transferred" in {
        val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(MarriageAllowanceTransferred, Some(234), 11500, "MarriageAllowanceTransferred"), employmentNames, companyCarBenefits, totalTax)
        val url = routes.ExternalServiceRedirectController.auditInvalidateCacheAndRedirectService(TaiConstants.MarriageAllowanceService).url
    ...
    "build a displayable Car Benefit LinkViewModel" which {
      "has the employment id added to it" when {
        "tax component is CarBenefit and employment id is present" in {
          val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(CarBenefit, Some(234), 11500, "CarBenefit"), employmentNames, companyCarBenefits, totalTax)
      ...
      "has zero as employment id" when {
        "tax component is CarBenefit and employment id is NOT present" in {
          val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(CarBenefit, None, 11500, "CarBenefit"), employmentNames, companyCarBenefits, totalTax)
    ...
    "build a displayable Company Benefit LinkViewModel" which {
      "has the employment id and benefit type added to it" when {
        "coding component is of type BenefitComponentType" in {
          val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(TaxableExpensesBenefit, Some(234), 11500, "TaxableExpensesBenefit"), employmentNames, companyCarBenefits, totalTax)
    ...
    "build a displayable Car Fuel Benefit LinkViewModel" which {
      "has the employment id added to it" when {
        "tax component CarFuelBenefit and employment id is present" in {
          val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(CarFuelBenefit, Some(234), 11500, "CarFuelBenefit"), employmentNames, companyCarBenefits, totalTax)
          val url = s"${ApplicationConfig.updateCompanyCarDetailsUrl}/234"
      ...
      "has zero as employment id" when {
        "tax component CarFuelBenefit and employment id is NOT present" in {
          val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(CarFuelBenefit, None, 11500, "CarFuelBenefit"), employmentNames, companyCarBenefits, totalTax)
          val url = s"${ApplicationConfig.updateCompanyCarDetailsUrl}/0"
          row.link mustBe ChangeLinkViewModel(true, Messages("tai.taxFreeAmount.table.taxComponent.CarFuelBenefit"), url)
        }
      }
    ...
    "build a displayable Allowance LinkViewModel" when {
      "coding component is of type AllowanceComponentType" in {
        val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(PersonalPensionPayments, Some(1), 1, "PersonalPensionPayments"), employmentNames, companyCarBenefits, totalTax)
    ...
    "display the company car make and model" when {
      "the coding component type is company car benefit" in {
        val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(CarBenefit, Some(10), 11500, "CarBenefit"), employmentNames, companyCarBenefits, totalTax)
      ...
      "employment id does not exist in company car benefits" in {
        val row = TaxFreeAmountSummaryRowViewModel(CodingComponent(CarBenefit, Some(1), 11500, "CarBenefit"), employmentNames, companyCarBenefits, totalTax)
