...
class TaxCodePairsSpec extends PlaySpec{
  ...
  "TaxCodePairs" should {
    ...
    "return pairs based on employment name when duplicate payroll numbers exist" in {
      ...
      val secondaryEmployer1Before = TaxCodeRecord("code", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation,"Employer 1", false, Some("A-1234"), false)
      val secondaryEmployer2Before = TaxCodeRecord("code", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation,"Employer 2", false, Some("A-1234"), false)
    ...
    "return pairs based on employment name when no payroll numbers exist for one employment name" in {
      val dateOfTaxCodeChange = startDate.plusMonths(1)
      val secondaryEmployer1Before = TaxCodeRecord("code", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation, "Employer 1", false, None, false)
      val secondaryEmployer2Before = TaxCodeRecord("code", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation,"Employer 2", false, None, false)
      val secondaryEmployer1After = secondaryEmployer1Before.copy(startDate =  startDate.plusMonths(1), endDate = TaxYearResolver.endOfCurrentTaxYear)
      val secondaryEmployer2After = secondaryEmployer2Before.copy(startDate =  startDate.plusMonths(1), endDate = TaxYearResolver.endOfCurrentTaxYear)
    ...
    "return no duplicating pairs when payroll numbers don't exists but employment names are different" in {
      ...
      val secondaryEmployer1Before = TaxCodeRecord("code", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation,"Employer 1", false, None, false)
      val secondaryEmployer2Before = TaxCodeRecord("code", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation,"Employer 2", false, None, false)
      val secondaryEmployer3After = TaxCodeRecord("code", startDate.plusMonths(1), TaxYearResolver.endOfCurrentTaxYear, OtherBasisOperation,"Employer 3", false, None, false)
      val secondaryEmployer4After = TaxCodeRecord("code", startDate.plusMonths(1), TaxYearResolver.endOfCurrentTaxYear, OtherBasisOperation,"Employer 4", false, None, false)
    ...
    "return non duplicating pairs when payroll numbers don't exist and employment name is the same by split by primary and secondary" in {
      val dateOfTaxCodeChange = startDate.plusMonths(1)
      val primaryEmployer1Before = TaxCodeRecord("code", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation,"Employer 1", false, None, true)
      val secondaryEmployer2Before = TaxCodeRecord("code", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation,"Employer 1", false, None, false)

      val primaryEmployer1After = TaxCodeRecord("code", startDate.plusMonths(1), TaxYearResolver.endOfCurrentTaxYear, OtherBasisOperation,"Employer 1", false, None, true)
      val secondaryEmployer2After = TaxCodeRecord("code", startDate.plusMonths(1), TaxYearResolver.endOfCurrentTaxYear, OtherBasisOperation,"Employer 1", false, None, false)
    ...
    "return the best possible match when multiple payroll numbers are missing for a employer" in {
      val dateOfTaxCodeChange = startDate.plusMonths(1)
      val secondaryEmployer1ABefore = TaxCodeRecord("code 1a", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation,"Employer 1", false, None, false)
      val secondaryEmployer1BBefore = TaxCodeRecord("code 1b", startDate, dateOfTaxCodeChange.minusDays(1), OtherBasisOperation,"Employer 1", false, None, false)

      val secondaryEmployer1AAfter = TaxCodeRecord("code 1a - after", dateOfTaxCodeChange, TaxYearResolver.endOfCurrentTaxYear, OtherBasisOperation, "Employer 1", false, None, false)
      val secondaryEmployer1BAfter = TaxCodeRecord("code 1b - after", dateOfTaxCodeChange, TaxYearResolver.endOfCurrentTaxYear, OtherBasisOperation, "Employer 1", false, None, false)
  ...
  val previousTaxCodeRecord1 = TaxCodeRecord("code", startDate, startDate.plusMonths(1), OtherBasisOperation,"A Employer 1", false, Some("A-1234"), false)
  ...
  val fullYearTaxCode = TaxCodeRecord("code", startDate, TaxYearResolver.endOfCurrentTaxYear, OtherBasisOperation, "B Employer 1", false, Some("B-1234"), false)
  val primaryFullYearTaxCode = fullYearTaxCode.copy(employerName = "C", payrollNumber = Some("C-1234"), primary = true)
  val unmatchedPreviousTaxCode = TaxCodeRecord("Unmatched Previous", startDate, startDate.plusMonths(1), OtherBasisOperation,"D", false, Some("D Payroll Id"), false)
  val unmatchedCurrentTaxCode = TaxCodeRecord("Unmatched Current", startDate.plusMonths(1), TaxYearResolver.endOfCurrentTaxYear, OtherBasisOperation,"E", false,Some("E Payroll id"), false)
