  ...
  "latestAnnualAccount" must {
    "return the latest annual account" when {
      "there are multiple annual accounts" in {
        val employment = Employment("", None, new LocalDate(), None, List(annualAccount1, annualAccount2), "", "", 1, None, false, false)

        employment.latestAnnualAccount mustBe Some(annualAccount2)

      }
      "there is only one annual account" in {
        val employment = Employment("", None, new LocalDate(), None, List(annualAccount1), "", "", 1, None, false, false)
