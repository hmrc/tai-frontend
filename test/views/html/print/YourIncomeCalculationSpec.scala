...
class YourIncomeCalculationSpec
  ...
  "The income calculation print page" should {
    "show the necessary back and print buttons" in {
      ...
      val doc = Jsoup.parse(views.html.print.yourIncomeCalculation(incomeCalcViewModel, empId = Some(2))(request = FakeRequest("GET", routes.YourIncomeCalculationController.printYourIncomeCalculationPage(None).url), messages = play.api.i18n.Messages.Implicits.applicationMessages, user = builders.UserBuilder.apply()).toString())
