...
class IncomeTaxComparisonControllerSpec extends BaseSpec {
  ...
  "onPageLoad" must {
    "display the cy plus one page" in {
      ...
      verify(employmentService).employments(any(), meq(TaxYear()))(any(), any())
    }
  ...
  "rendered CY+1 page" must {
      ...
      when(employmentService.employments(any(), meq(TaxYear()))(any(), any()))
        .thenReturn(
          EitherT[Future, UpstreamErrorResponse, Seq[Employment]](
            Future.successful(Right(Seq(employment, employment2)))
          )
        )
      val result = controller.onPageLoad()(request)
      status(result) mustBe OK
    ...
    "show estimated income for CY and CY+1 for multiple pensions" in {
      ...
      when(employmentService.employments(any(), meq(TaxYear()))(any(), any()))
        .thenReturn(
          EitherT[Future, UpstreamErrorResponse, Seq[Employment]](
            Future.successful(Right(Seq(employment, employment2, pension, pension2)))
          )
        )
      val result = controller.onPageLoad()(request)
      status(result) mustBe OK
  ...
  class TestController()
    ...
    when(employmentService.employments(any(), meq(TaxYear()))(any(), any()))
      .thenReturn(
        EitherT[Future, UpstreamErrorResponse, Seq[Employment]](
          Future.successful(Right(Seq(employment)))
        )
      )
