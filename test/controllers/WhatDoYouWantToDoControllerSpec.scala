...
class WhatDoYouWantToDoControllerSpec extends BaseSpec with JsoupMatchers {
  ...
  "Calling the What do you want to do page method" must {
    ...
    "return the general 500 error page" when {
      ...
      "income tax history toggle lookup fails" in {
        val testController = createTestController()
        ...
        when(taxAccountService.newTaxCodeIncomes(any(), any())(any()))
          .thenReturn(EitherT.rightT(Seq.empty[TaxCodeIncome]))
        when(mockFeatureFlagService.getAsEitherT(org.mockito.ArgumentMatchers.eq(IncomeTaxHistoryToggle)))
    ...
    "display the WDYWTD page (not redirect)" when {
      ...
      "employments hod call has returned not found for current year, " +
        "but CY-1 returns an error so entry is allowed immediately" in {
      ...
      "current year employments call returns an error, so entry is allowed" in {
        ...
        status(result) mustBe OK
        verify(employmentService, times(2)).employments(any(), meq(TaxYear()))(any())
        verify(employmentService, times(0)).employments(any(), meq(TaxYear().prev))(any())
