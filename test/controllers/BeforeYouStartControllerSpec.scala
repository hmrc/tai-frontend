...
class BeforeYouStartControllerSpec extends BaseSpec {
  ...
  "BeforeYouStartController.onPageLoad" must {
    ...
    "return NotFound for an invalid journey type" in {
      val result = sut.onPageLoad("not-a-journey")(RequestBuilder.buildFakeRequestWithAuth("GET"))

      status(result) mustBe NOT_FOUND
      contentAsString(result) must include("Invalid journey type")
    }
  }
}
